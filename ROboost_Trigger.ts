...
const SSName1 = process.env.RROboost_Trigger_SheetName;
  ...
  try {
    ...
    if (WorkStatus[0] == 'good') {
      ...
      await RPA.Slack.chat.postMessage({
        channel: BotChannel,
        token: BotToken,
        text: `インフィード入札額 当日トリガー調整 問題なく完了しました`,
        icon_emoji: ':snowman:',
        username: 'p1'
      });
    }
    if (WorkStatus[0] == 'error') {
      ...
      await RPA.Slack.chat.postMessage({
        channel: BotChannel,
        token: BotToken,
        text: `インフィード入札額 当日トリガー調整 エラーが発生しました\n@kushi_makoto 確認してください`,
        icon_emoji: ':snowman:',
        username: 'p1'
      });
    }
...
async function TargetInputSelect(SheetData, SheetWorkingRow) {
  const NextPageButton = await RPA.WebBrowser.findElementByCSSSelector(
    '#main > article > div.contents.ng-scope > section > div:nth-child(7) > div > div:nth-child(2) > div > ul > li.pagination-next.ng-scope > a'
  );
  for (let v = 0; v < 5; v++) {
