...
const SSName1 = process.env.ROboost_0000_SheetName;
  ...
  try {
    ...
    if (WorkStatus[0] == 'good') {
      ...
      await RPA.Slack.chat.postMessage({
        channel: BotChannel,
        token: BotToken,
        text: `インフィード入札額 0時調整 問題なく完了しました`,
        icon_emoji: ':snowman:',
        username: 'p1'
      });
    }
    if (WorkStatus[0] == 'error') {
      ...
      await RPA.Slack.chat.postMessage({
        channel: BotChannel,
        token: BotToken,
        text: `インフィード入札額 0時調整 エラーが発生しました\n@kushi_makoto 確認してください`,
        icon_emoji: ':snowman:',
        username: 'p1'
      });
    }
...
async function AJALogin() {
  await RPA.WebBrowser.get(
    'https://agency.aja.fm/#/campaign?agency_id=1210&advertiser_id=8919'
  );
...
async function PageMoveing(SheetData) {
...
async function TargetInputSelect(SheetData, SheetWorkingRow) {
  const List = await RPA.WebBrowser.findElementsByClassName('select');
  for (let i in List) {
    ...
    try {
      ...
      // IDが見つからない時は A列をエラー表示に変更し、Slackにてスキップ報告を行う
      await RPA.Google.Spreadsheet.setValues({
        spreadsheetId: `${SSID}`,
        range: `${SSName1}!A${SheetWorkingRow[0]}:A${SheetWorkingRow[0]}`,
        values: [['ID不一致']]
      ...
      await RPA.Slack.chat.postMessage({
        channel: BotChannel,
        token: BotToken,
        text: `インフィード入札額 0時調整 IDが一致しませんでした → ${SheetData[2]}\nスキップして次の作業に移ります`,
        icon_emoji: ':snowman:',
        username: 'p1'
      });
      break;
