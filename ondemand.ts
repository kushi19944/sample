    ...
    for (let i in ItemGroupUrl) {
      ...
      for (let k = 0; k < ModuleCount.length; k++) {
        ...
        if (text2 != '入稿物') {
          ...
          //const SortJun = await RPA.WebBrowser.findElementByXPath('//*[@id="root"]/div/section/section/div/div[2]/form/div[8]/div[2]/div[2]/div/div[2]/div/span[1]/span/span[1]/span');
          // ソート順　をクリックして priority を入力する
          await RPA.WebBrowser.mouseClick(OpeInput[1]);
          await RPA.sleep(800);
          const SortInput = await RPA.WebBrowser.findElementByCSSSelector(
            'body > span > span > span.select2-search.select2-search--dropdown > input'
          );
          await RPA.WebBrowser.sendKeys(SortInput, ['priority']);
          await RPA.WebBrowser.sendKeys(SortInput, [RPA.WebBrowser.Key.ENTER]);
          // 最低保証件数 を入力する
