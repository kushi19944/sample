...
async function CardCreate(CardData){
    ...
    if(CardData[6][1] == undefined){
        const TitleText = await RPA.WebBrowser.driver.executeScript(`document.getElementById("annotator-prs-title-associated").value`);
        RPA.Logger.info(TitleText);
        const TitleText2 = await CardTitleInput.getText();
        RPA.Logger.info(TitleText2);
        await RPA.Logger.info(TitleText2);
        if(String(TitleText).length > 50){
