...
async function Start(){
    ...
    const SheetDatas1 = [
    ["false",["URLor再生リスト"],["秒数"],["URL"],["フレーズ文言"],["ティーザー文言"],["検索タイトル・カードタイトル"],["設定タイトル/詳細"],["固定コメント"]],
    ["false",["URLor再生リスト"],["秒数"],["URL"],["フレーズ文言"],["ティーザー文言"],["検索タイトル・カードタイトル"],["設定タイトル/詳細"],["固定コメント"]],
    ["false",["URLor再生リスト"],["秒数"],["URL"],["フレーズ文言"],["ティーザー文言"],["検索タイトル・カードタイトル"],["設定タイトル/詳細"],["固定コメント"]]];
    const SheetDatas2 = [
    ["false",["URLor再生リスト"],["秒数"],["URL"],["フレーズ文言"],["ティーザー文言"],["検索タイトル・カードタイトル"],["設定タイトル/詳細"],["固定コメント"]],
    ...
    // 2回目は特殊な方法でログインする必要あるので下記でログイン処理
...
async function YoutubeLogin(Account){
    ...
    const GoogleLoginID = await RPA.WebBrowser.wait(RPA.WebBrowser.Until.elementLocated({xpath:'//*[@id="identifierId"]'}),100000);
...
async function Youtube2ndLogin(Account){
    ...
    await RPA.sleep(5000);
    const AccountButton = await RPA.WebBrowser.wait(RPA.WebBrowser.Until.elementLocated({id:'avatar-btn'}),100000);
    ...
    await RPA.sleep(1000);
    const LoginID = await RPA.WebBrowser.wait(RPA.WebBrowser.Until.elementLocated({id:'identifierId'}),5000);
    ...
    await RPA.sleep(1000);
    const LoginPW = await RPA.WebBrowser.wait(RPA.WebBrowser.Until.elementsLocated({className:'whsOnd zHQkBf'}),5000);
    await RPA.WebBrowser.sendKeys(LoginPW[0],[Account[2]]);
...
async function TitleSearch(TargetTitle){
    const SearchInput = await RPA.WebBrowser.wait(RPA.WebBrowser.Until.elementLocated({id:'query-input'}),5000);
    await RPA.sleep(1000);
...
async function CardPage(){
    // タイトルを取得する
    const target = await RPA.WebBrowser.wait(RPA.WebBrowser.Until.elementsLocated({className:"style-scope ytcp-img-with-fallback"}),5000);
    for(let i in target){
        const data = await RPA.WebBrowser.findElementByXPath(`/html/body/ytcp-app/ytcp-entity-page/div/div/main/div/ytcp-animatable[3]/ytcp-video-section/ytcp-video-section-content/div/ytcp-video-row[${Number(i)+Number(1)}]/div/div[2]/ytcp-video-list-cell-video/div[2]/h3/a`);
        const Image = await RPA.WebBrowser.findElementByXPath(`/html/body/ytcp-app/ytcp-entity-page/div/div/main/div/ytcp-animatable[3]/ytcp-video-section/ytcp-video-section-content/div/ytcp-video-row[${Number(i)+Number(1)}]/div/div[2]/ytcp-video-list-cell-video/div[1]/a/div/ytcp-thumbnail/div/ytcp-img-with-fallback/div/img`);
        const targetText = await data.getText();
        if(targetText == `※使用しない※　25_YS7773_190305_チャンスの時間_40_第3回モノマネ50音選手権｜AbemaTV バラエティCh`){
            await RPA.sleep(300);
            await RPA.WebBrowser.mouseClick(Image);
            break
        }
    }
    await RPA.sleep(1500);
    const card = await RPA.WebBrowser.wait(RPA.WebBrowser.Until.elementLocated({xpath:'/html/body/ytcp-app/ytcp-entity-page/div/div/main/div/ytcp-animatable[8]/ytcp-video-metadata-editor-section/ytcp-video-metadata-editor/div/ytcp-animatable/ytcp-video-metadata-basics/div/div[2]/a[1]/ytcp-text-dropdown-trigger/ytcp-dropdown-trigger/div/div[2]/span'}),5000);
...
async function TitleSyousaiChange(CardData){
    ...
    await RPA.sleep(2000);
    const titleinput = await RPA.WebBrowser.wait(RPA.WebBrowser.Until.elementLocated({xpath:'/html/body/ytcp-app/ytcp-entity-page/div/div/main/div/ytcp-animatable[8]/ytcp-video-metadata-editor-section/ytcp-video-metadata-editor/div/ytcp-animatable/ytcp-video-metadata-basics/div/div[1]/div[1]/ytcp-mention-textbox/ytcp-form-input-container/div[1]/div[2]/ytcp-mention-input/div'}),5000);
    const syousaiinput = await RPA.WebBrowser.findElementByXPath('/html/body/ytcp-app/ytcp-entity-page/div/div/main/div/ytcp-animatable[8]/ytcp-video-metadata-editor-section/ytcp-video-metadata-editor/div/ytcp-animatable/ytcp-video-metadata-basics/div/div[1]/div[2]/ytcp-mention-textbox/ytcp-form-input-container/div[1]/div[2]/ytcp-mention-input/div');
    await titleinput.clear();
    await RPA.sleep(300);
    // 動画タイトルを入力
    await RPA.WebBrowser.sendKeys(titleinput,[CardData[7][0]]);
    await syousaiinput.clear();
    await RPA.sleep(300);
    // 詳細を入力
    await RPA.WebBrowser.sendKeys(syousaiinput,[CardData[7][1]]);
